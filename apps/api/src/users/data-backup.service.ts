...
export class DataBackupService {
  ...
  public async backupDataForUser(id: UUID): Promise<void> {
    ...
    result.user = user
    result.shortcuts = await this.shortcutRepository.find({ userId: id })
    result.notes = await this.noteRepository.find({ userId: id })
    result.personalData = await this.personalDataRepository.find({ userId: id })
