...
export class PersonalDataService {
  ...
  public async getAllForUser(userId: UUID): Promise<PersonalDataDto[]> {
    const results = await this.personalDataRepository.find({ userId })
    if (!results.length) {
  ...
  public async updatePersonalData(
    ...
    const personalData = await this.personalDataRepository.findOne({ id: request.id })
    if (!personalData) {
      throw new NotFoundException(CustomApiError.ITEM_NOT_FOUND)
    }
  ...
  public async deletePersonalData(
    ...
    const item = await this.personalDataRepository.findOne({ id })
    if (!item) {
      throw new NotFoundException(CustomApiError.ITEM_NOT_FOUND)
    }
